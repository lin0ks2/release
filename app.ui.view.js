...
(function(){
  ...
  }catch(e){ console.warn('Info modal safe patch error', e); }
})();

// Enhanced Info modal bootstrap: tabs + stable height
(function(){
  const modal     = document.getElementById('infoModal');
  const titleEl   = document.getElementById('infoTitle');
  const contentEl = document.getElementById('infoContent');
  ...
  function t(){
    try{ return (typeof App!=='undefined' && typeof App.i18n==='function') ? (App.i18n()||{}) : {}; }catch(_){ return {}; }
  }

  function fillInstruction(){
    try{
      const bag = t();
      if (titleEl && bag.infoTitle) titleEl.textContent = bag.infoTitle;
      if (tabInstruction && bag.tabInstruction) tabInstruction.textContent = bag.tabInstruction;
      if (tabAbout && bag.tabAbout) tabAbout.textContent = bag.tabAbout;

      if (Array.isArray(bag.infoSteps) && contentEl){
        contentEl.innerHTML = '';
        const ul = document.createElement('ul');
        bag.infoSteps.forEach(function(s){
          const li = document.createElement('li'); li.textContent = String(s||''); ul.appendChild(li);
        });
        contentEl.appendChild(ul);
      }
    }catch(_){}
  ...
  function fillAbout(){
    try{
      const bag = t();
      const ver = (typeof App!=='undefined' && App.APP_VER) ? String(App.APP_VER) : '—';
      const isReg = (localStorage.getItem('lexitron.regKey')||'').trim().length>0;
      const status = isReg ? (bag.aboutRegistered||'Зареєстровано') : (bag.aboutNotRegistered||'Не зареєстровано');
      const vEl = document.getElementById('aboutVersion');
      const sEl = document.getElementById('aboutStatus');
  ...
  function showTab(which){
    ...
    tabInstruction.classList.toggle('active', !isAbout);
    tabInstruction.setAttribute('aria-selected', String(!isAbout));
    tabAbout.classList.toggle('active', isAbout);
    tabAbout.setAttribute('aria-selected', String(isAbout));
    panelInstruction.classList.toggle('hidden', isAbout);
    panelAbout.classList.toggle('hidden', !isAbout);
    if (isAbout) fillAbout();
  }
  ...
  function closeInfo(){ modal && modal.classList.add('hidden'); }

  if (tabInstruction) tabInstruction.addEventListener('click', function(){ showTab('instruction'); });
  if (tabAbout) tabAbout.addEventListener('click', function(){ showTab('about'); });
  if (closeEl) closeEl.addEventListener('click', closeInfo);
  if (okEl) okEl.addEventListener('click', closeInfo);

  const btnCheck = document.getElementById('btnCheckUpdates');
  if (btnCheck){ btnCheck.addEventListener('click', function(){ alert((t().checkUpdatesDone)||'Перевірка оновлень…'); }); }
  const btnReg = document.getElementById('btnRegister');
  if (btnReg){
    btnReg.addEventListener('click', function(){
      const val = (document.getElementById('regKey')||{}).value||'';
      localStorage.setItem('lexitron.regKey', val.trim());
      fillAbout();
      alert((t().registerSaved)||'Ключ збережено');
    });
  }

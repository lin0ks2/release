...
function current() {
    ...
    // Нормализуем абсолютный индекс App.state.index к локальному индексу активного набора
    try {
      if (App && App.Sets && typeof App.Sets.activeBounds === 'function') {
        const b = App.Sets.activeBounds();
        let idxAbs = Number(App.state.index) || 0;
        let idxLocal = idxAbs - (b.start|0);
        const len = Math.max(0, (b.end|0) - (b.start|0)) || deck.length;
        if (idxLocal < 0 || idxLocal >= len) {
          // прижимаем к началу набора и синхронизируем абсолютный индекс
          idxLocal = 0;
          App.state.index = (b.start|0);
          if (typeof App.saveState === 'function') App.saveState();
        }
        return deck[idxLocal];
      }
    } catch(e) {}

    // Фолбэк: трактуем индекс как локальный (как было)
...
function updateStats() {
  try {
    ...
    // Спец-ветка для "mistakes": считаем по их внутреннему хранилищу
    if (key === 'mistakes' && App.Mistakes && App.Mistakes.getStars) {
      ...
      // 1) Считаем полностью завершённые наборы
      ...
      for (let si = 0; si < totalSets; si++) {
        if (doneMap[si]) {
          ...
          learned += (end - start); // весь набор засчитывается
        }
      }

      // 2) Добавляем прогресс по текущему набору (по звёздам scoped)
      const activeSet = (App.Sets && App.Sets.getActiveSetIndex) ? App.Sets.getActiveSetIndex() : 0;
      if (!doneMap[activeSet]) { // чтобы не удвоить, если он уже в done
    ...
    // тихо, чтобы не падать из-за мелочей
  }
}
  ...
  function makeDictRow(key) {
    ...
    if (key === 'fav' || key === 'favorites') {
      ...
      delBtn.addEventListener('click', (e) => {
        ...
        // безопасный выбор дефолтного словаря вместо "App.Decks./* ... */"
        var defKey = null;
        try {
  ...
  App.bootstrap = function () {
    ...
    if (!App.dictRegistry.activeKey) {
      // безопасный дефолт вместо "App.Decks./* ... */"
      var defKey = null;
      try {
        if (App.Decks && typeof App.Decks.pickDefaultKey === 'function') {
          defKey = App.Decks.pickDefaultKey();

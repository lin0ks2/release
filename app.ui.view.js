...
/* === [PATCH 2025-10-14] Info modal tabs balance patch — avoid jumping === */
(function(){
  try{
    var modal = document.getElementById('infoModal');
    if(!modal) return;
    ...
    function measure(el){
      ...
      if(hidden){
        ...
        el.style.left='-9999px'; el.style.display='block';
      }
    ...
    function balance(){
      var h1 = measure(document.getElementById('panel-instr'));
      var h2 = measure(document.getElementById('panel-about'));
      var mh = Math.max(h1||0, h2||0);
      if(mh>0) body.style.minHeight = mh + 'px';
    }
    var run = function(){ if(!modal.classList.contains('hidden')) balance(); };
    run();
    try{
      var obs = new MutationObserver(run);
      obs.observe(modal, {attributes:true, attributeFilter:['class']});
    }catch(_){}
    var tabs = modal.querySelector('#infoTabs');
    if(tabs) tabs.addEventListener('click', function(e){
      var id = e.target && e.target.id;
      if(id==='tab-instr' || id==='tab-about') setTimeout(balance, 0);
    }, {passive:true});
    window.addEventListener('resize', balance, {passive:true});
  }catch(e){ console.warn('tabs balance patch error', e); }
})();
